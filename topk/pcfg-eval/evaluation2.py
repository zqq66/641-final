...
def main(args):
    pred_spans_file = "./topk_span/my_out/top100_span_trees.pkl"
    with open(pred_spans_file, 'rb') as f:
        all_pred_spans = pickle.load(f)
        print(11111, len(all_pred_spans))
    ...
    for idx, tree in enumerate(open(args.data_file, "r")):
        ...
        max_fn = -1
        all_f1 = []
        print(len(all_pred_spans[idx-remove]))
        ...
        for pred_span in all_pred_spans[idx-remove]:
            # pred_span = all_pred_spans[idx-remove][i]
            num_sents += 1
